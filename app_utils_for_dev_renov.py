...
def rid_blank(snts):
    ...
    return sents
    """
    snts = [p.sub(' ',z2.sub('\g<qt2> ',z1.sub(' \g<qt1>',p.sub(' ',s)))).strip() for s in sents]    
...
def to_normal(sents):
    ...
    snts = [p1.sub(' ',z2.sub('\g<qt2> ',z1.sub(' \g<qt1>',p1.sub(' ',s)))).strip() for s in snts]    
    #snts = p1.sub(' ','Æ'.join(snts)) 
