...
from trns.utils import get_sents_lenth4_new, get_sents_lenth_new, get_notEn
...
class NMT(nn.Module):
    ...
    def parallel_encode_new(self,source, lang, mapping=False): #slang_is_tlang=False):
    ...
    def ek_beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70, tlang:str='en') -> List[Khypothesis]:
        ...
        for i,v in enumerate(completed_hypotheses):
            if completed_hypotheses[i].score/len(completed_hypotheses[i].xo) > best_score:
                ...
                k_best = i
                sent_len = len(completed_hypotheses[i].xo)
        print("k_best ;{},  sent_len : {}".format(k_best, sent_len))
